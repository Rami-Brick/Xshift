...
export async function POST(request: NextRequest) {
  ...
  const { email, password, full_name, work_start_time, work_end_time, leave_balance, role, default_day_off } = parsed.data;
  ...
  const { error: profileError } = await service.from('profiles').upsert({
    ...
    email,
    role: role ?? 'employee',
    work_start_time: work_start_time ?? settings?.default_work_start_time ?? '09:00',
    work_end_time: work_end_time ?? settings?.default_work_end_time ?? '18:00',
    leave_balance: leave_balance ?? 30,
